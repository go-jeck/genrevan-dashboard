  ...
  def detail
    begin
      ...
      for lxc_json in lxcs_json do
        lxc = Lxc.new(lxc_json["id"], lxc_json["name"], lxc_json["ip_address"], lxc_json["image"], lxc_json["status"], lxc_json["lxd_id"], lxc_json["host_port"], lxc_json["container_port"], lxc_json["error_message"])
        @lxcs.push(lxc)
      end
    rescue Errno::ECONNREFUSED
      render :file => 'public/503.html', :status => :error, :layout => false
